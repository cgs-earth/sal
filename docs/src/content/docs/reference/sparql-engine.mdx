...
A projected object variable is read back as `COALESCE(object_iri, CAST(object_float AS VARCHAR), object_string)`, so a result row shows the object whichever column holds it.

:::caution
`object_geometry` is not part of that coalesce. A triple whose object is a GeoSPARQL WKT literal stores its geometry there and leaves the other object columns null, so binding such an object in SPARQL yields an empty value. Read geometries through SQL and the `ST_` functions instead — `sal query`, `POST /api/sql`, or `GET /geometries` — or restore them as WKT literals with [`sal export`](/sal/reference/commands/#sal-export).
:::

In a `FILTER`, the same rule applies to whichever side the variable is compared against, so `FILTER(?age > 21)` compares `object_float` while `FILTER(?name = "bob")` compares `object_string`.
...
| Functions such as `REGEX`, `STR`, `LANG`, `BOUND`, `ST_*` | No |
...
Because results come back from SQL as text, a binding's `type` is inferred from its value rather than carried from the table: a value starting with `http://` or `https://` is reported as a `uri`, one starting with `_:` as a `bnode`, and everything else as a `literal`. Datatypes and language tags are not reported.

## When the subset is not enough

Two ways around it, depending on what you need:

- **Query the table as SQL.** [`sal query`](/sal/reference/commands/#sal-query) opens the same `triples` view in a DuckDB shell, and `sal serve --with-ui` exposes `POST /api/sql`. Aggregation, ordering, window functions, geometry via the `ST_` functions, and joins against imported data products are all available there. A project that imported a data product with `sal import oci://...` gets a view per imported table plus an `imports` view stacking them, none of which SPARQL can reach — SPARQL only ever queries the project's own `triples` view.
