...
4. **Execute.** DuckDB runs the statement against the Iceberg table through the `triples` view, reading the current snapshot. A query sent to `sal serve`'s `/v{snapshot}/sparql` route scans the table at that snapshot instead: each triple pattern becomes an `iceberg_scan(..., snapshot_from_id = {snapshot})` in place of the view, so the translated SQL says which snapshot it reads. DuckDB is linked into the `sal` binary, so no external database or `duckdb` CLI is involved.
...
RDFS reasoning is accomplished by writing a property path in the query.
`sal build` does not add inferred triples to the table, keep a separate index of class or property hierarchies, or expand a query behind the scenes; the closure of `rdfs:subClassOf` is computed by DuckDB when the query runs, from the rows the table holds.
Those rows are what the project's own RDF asserts and every ontology it imports with `owl:imports`, which [`sal build`](/sal/reference/commands/#sal-build) merges into the data product.

For example, a project that imports schema.org can ask for the instances of every class below `schema:Organization`:
...
`rdfs:subPropertyOf`, `rdfs:domain`, and `rdfs:range` are walked the same way, `?x ?p ?o . ?p rdfs:domain/rdfs:subClassOf* schema:Organization`, so the RDFS entailments a project needs are written in the query rather than switched on as a regime.

A vocabulary the project only validates against, one it declares a prefix for but never imports, has no statements in the table, only the provenance node recording which version was pinned.
The path above finds nothing through it: a hierarchy has to be imported before a query can walk it.
That keeps a query answerable from the data product alone, and keeps a vocabulary's own instances and class declarations, hundreds of them in schema.org, out of every result unless the project chose to carry them.
