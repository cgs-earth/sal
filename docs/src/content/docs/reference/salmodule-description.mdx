...
A task may also produce files. An IRI written with the `file:///` scheme, such as `{"@id": "file:///tmp/test.txt"}`, names a file inside the container that the SAL Project copies out while the task runs, stores by the SHA-256 digest of its contents, and refers to as `urn:sha256:<digest>` in place of the path, with the file's name recorded as `rdfs:label`, its modification time as `dcterms:modified`, its digest as `owl:versionIRI`, and its path under the project's blob store as `dcterms:identifier`. Any other statement the task makes about the `file:///` IRI is kept under the new name. An IRI ending in a slash, such as `{"@id": "file:///out/catalog/"}`, names a directory that is copied whole and stored under its own path rather than its digest, so that a Zarr store or STAC catalog is readable in place; the digest of its contents is recorded beside it in the blob store's `prov.jsonld` and is what the graph refers to it by. Since the copy happens as soon as the line naming the path is read from stdout, a task must write a file, or every file in a directory, completely before it prints the node that refers to it. Each path is copied once; further references to it are skipped.
