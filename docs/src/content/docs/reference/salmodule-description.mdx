...
A task may also produce files. An IRI object written with the `file:///` scheme, such as `{"@id": "file:///tmp/test.txt"}`, names a file inside the container that the SAL Project copies out while the task runs, stores by the SHA-256 digest of its contents, and refers to as `urn:sha256:<digest>` in place of the path, with the file's name recorded as `rdfs:label`, its modification time as `dcterms:modified`, and its digest as `owl:versionIRI`. Since the copy happens as soon as the line naming the file is read from stdout, a task must write a file completely before it prints the node that refers to it. Each file is copied once; further references to the same path are skipped.
