---
title: Semantic Accessibility Layer (SAL) System
description: The SAL vision, the SAL Module specification, and its command line interface usage.
---
...
`docker run -e SALMODULE_TASK_INSTANCE="<sal:NodeProcessor node object instance>" MYIMAGE sal run`
...
```
$ SALMODULE_TASK_INSTANCE='{"@id":"http://example.org/#pistolpete","@type":"EducationalHistoryFinder"}' 
$ docker run MYIMAGE sal run -e SALMODULE_TASK_INSTANCE="${SALMODULE_TASK_INSTANCE}" 
...
The output is instance data belonging to the SAL Project that ran the task, so the base IRI of the JSON-LD a task emits is the project's namespace. A relative IRI in the output resolves against the project, not against the module's `salmodule://` namespace, which names the module's vocabulary rather than what it produces. A module that needs to name its output under a specific namespace, such as the authoritative source the data comes from, emits absolute IRIs.

A task may also produce files. An IRI written with the `file:///` scheme, such as `{"@id": "file:///tmp/test.txt"}`, names a file inside the container that the SAL Project copies out while the task runs, stores by the SHA-256 digest of its contents, and refers to as `urn:sha256:<digest>` in place of the path, with the file's name recorded as `rdfs:label`, its modification time as `dcterms:modified`, its digest as `owl:versionIRI`, its path under the project's blob store as `dcterms:identifier`, and the module as `dcterms:source`. Any other statement the task makes about the `file:///` IRI is kept under the new name. An IRI ending in a slash, such as `{"@id": "file:///out/catalog/"}`, names a directory that is copied whole and stored under `<digest>/` with its files verbatim, so that a Zarr store or STAC catalog is readable in place under a name no other module's output can collide with. Every copy is recorded in the blob store's `prov.jsonld`. Since the copy happens as soon as the line naming the path is read from stdout, a task must write a file, or every file in a directory, completely before it prints the node that refers to it. Each path is copied once; further references to it are skipped.

When the task's class declares a `salmodule:stdoutShape`, the SAL Project validates each line of stdout against that `sh:NodeShape` as it is read, with the module ontology's `@context` injected first. The first line that does not conform stops the container and fails the run with the shape's `sh:message`; nothing the task wrote is kept. A `salmodule:stdoutShape` may use any SHACL Core constraint except `sh:languageIn`, `sh:uniqueLang`, `sh:disjoint`, `sh:qualifiedValueShapesDisjoint`, and `sh:sparql`.
