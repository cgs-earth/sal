...
func wipe() error {
	...
	if err := removeCopiedFiles(blobsDir); err != nil {
		return err
	}
...
// removeCopiedFiles deletes the files and directories SAL module tasks handed
// over and the prov.jsonld that records them. They are artifacts of a run,
// regenerated by the next one, and once the table that refers to them is
// gone nothing resolves their digests. A run never removes them itself, so a
// wipe is the one place they go.
func removeCopiedFiles(blobsDir string) error {
	removed, err := salmodule.RemoveCopiedFiles(blobsDir)
	if err != nil {
		return err
	}
	if len(removed) > 0 {
		slog.Info(fmt.Sprintf("Removed %d files SAL module tasks copied into %s and their %s", len(removed), blobsDir, salmodule.ProvFile))
	}
	return nil
}
