...
func wipe() error {
	...
	if err := removeCopiedDirectories(blobsDir); err != nil {
		return err
	}
...
// removeCopiedDirectories deletes the directories SAL module tasks handed
...
// gone nothing resolves their digests.
func removeCopiedDirectories(blobsDir string) error {
	removed, err := salmodule.RemoveCopiedDirectories(blobsDir)
	if err != nil {
		return err
	}
	if len(removed) > 0 {
		slog.Info(fmt.Sprintf("Removed %d directories SAL module tasks copied into %s and their %s", len(removed), blobsDir, salmodule.ProvFile))
	}
	return nil
}
