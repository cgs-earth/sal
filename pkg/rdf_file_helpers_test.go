...
func TestFindRdfDataInPathsSkipsTheSalDirectory(t *testing.T) {
	...
	require.NoError(t, os.WriteFile(filepath.Join(project, ".sal", "ontology.ttl"), []byte("# managed by sal import\n"), 0644))
...
func TestFindRdfDataInPathsWalksTheSalDirectoryWhenItIsTheRequestedPath(t *testing.T) {
	...
	require.NoError(t, os.WriteFile(filepath.Join(salDir, "ontology.ttl"), []byte("# managed by sal import\n"), 0644))

	files, err := FindRdfDataInPaths([]string{salDir})

	require.NoError(t, err)
	require.Equal(t, []string{filepath.Join(salDir, "ontology.ttl")}, files)
}
